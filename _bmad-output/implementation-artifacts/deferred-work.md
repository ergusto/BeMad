...
- **Axe `label-content-name-mismatch` is blanket-filtered** — `tests/e2e/a11y.spec.ts` drops that rule for the whole page to allow the intended rotating-button Label-in-Name trade-off. After the sort-aria-label fix the only remaining mismatches are the intended rotating controls, but the blanket filter would also hide a *future accidental* mismatch on a non-rotating control. Consider tightening to an allowlist of the specific rotating control test-ids. [tests/e2e/a11y.spec.ts]
